...
async def main():
    ...
    group.add_argument(
        "--url",
        help="Scan a single target WordPress URL (https://example.com)"
    )
    group.add_argument(
        "--targets",
        help="Path to a TXT file containing multiple target URLs (one per line)"
    )
    parser.add_argument(
        "--threads",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of retries for failed HTTP requests (default: 2)."
    )
    parser.add_argument(
        "--update",
        action="store_true",
        help="Check for and provide instructions to update WPHunter to the latest version."
    )
    parser.add_argument(
        "--plugin",
        help="Scan for vulnerabilities specific to a given plugin slug ."
    )
    parser.add_argument(
        "--cve",
        help="Scan for a specific CVE ID ('CVE-2021-34624')."
    )
    parser.add_argument(
        "--save-by-plugin",
        action="store_true",
        help="Save a separate text file for each vulnerable plugin found. This saves only URLs, no other information."
    )
    ...
    if config.url:
        ...
        try:
            ...
            if not targets:
                logger.log(f"No target URLs found in '{config.targets_file}'. Please check the file and try again.", level='error')
                sys.exit(1)
        except FileNotFoundError:
            logger.log(f"Error: The target file '{config.targets_file}' was not found. Please provide a valid path.", level='error')
            sys.exit(1)
        except IOError as e:
